...
export function DesignationsTab({ token }: { token?: string | null }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const rows = await api<Designation[]>("/api/instructors/designations", { token: token ?? undefined });
      setDesignations(Array.isArray(rows) ? rows : []);
    } catch {
      snackbar.error("Could not load designations");
    } finally {
      setLoading(false);
    }
  }, [token, snackbar]);
  ...
  async function saveDesignation(e: React.FormEvent) {
    e.preventDefault();
    setSaving(true);
    try {
      const payload = {
      ...
      if (editing) {
        const updated = await api<Designation>(`/api/instructors/designations/${editing.id}`, {
        ...
        snackbar.success("Designation updated.");
      } else {
        const created = await api<Designation>("/api/instructors/designations", {
          token: token ?? undefined,
          method: "POST",
          body: JSON.stringify({ ...payload, is_active: true }),
        });
        setDesignations((prev) => [...prev, created].sort((a, b) => a.name.localeCompare(b.name)));
        snackbar.success("Designation created.");
      }
      setModal(false);
    } catch (err) {
  ...
  async function toggleActive(d: Designation) {
    ...
    setDesignations((prev) => prev.map((x) => (x.id === d.id ? { ...x, is_active: next } : x)));
    try {
      const updated = await api<Designation>(`/api/instructors/designations/${d.id}`, {
        token: token ?? undefined,
        method: "PATCH",
        body: JSON.stringify({ is_active: next }),
      });
      setDesignations((prev) => prev.map((x) => (x.id === updated.id ? updated : x)));
    } catch {
  ...
  async function removeDesignation(d: Designation) {
    if (!confirm(`Delete designation "${d.name}"?`)) return;
    try {
      await api(`/api/instructors/designations/${d.id}`, { token: token ?? undefined, method: "DELETE" });
      setDesignations((prev) => prev.filter((x) => x.id !== d.id));
      snackbar.success("Designation deleted.");
    } catch (err) {
      snackbar.error(err instanceof Error ? err.message : "Cannot delete designation");
    }
  }

  const meta = TAB_META.designations;
    ...
    <>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
        <Card className="py-3 px-4">
          ...
          <p className="text-xl font-bold mt-1">{designations.length}</p>
        </Card>
        <Card className="py-3 px-4">
      ...
      <Card className="p-0 overflow-hidden">
        {loading ? (
          ...
          <p className="p-6 text-sm text-[var(--muted)]">
            No designations yet. Use <strong>+ New designation</strong> to add titles such as Head Teacher, Senior
            Teacher, or Teaching Assistant. They appear as options when you add or edit teachers under{" "}
              ...
              <tbody>
                {designations.map((d) => (
                  <tr key={d.id} className="border-b border-[var(--border)] last:border-0 hover:bg-neutral-50/50">
                    ...
                    <td className="px-4 py-3">{d.teacher_count}</td>
                    ...
                    <td className="px-4 py-3 text-right">
                      ...
                      <button
                        ...
                        title={d.teacher_count > 0 ? "Reassign teachers first" : "Delete"}
                      >
                        Delete
                      </button>
                    </td>
      ...
      >
        <form onSubmit={saveDesignation} className="space-y-4">
          ...
          <div>
            ...
            <input type="text" value={code} onChange={(e) => setCode(e.target.value)} className={inputClass} placeholder="HT" />
          </div>
